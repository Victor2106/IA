...
module.exports = class Topdislike extends Command {
	constructor() {
		super({
			name: "topdislike",
			category: "music",
			aliases: ["topdislikes", "dislikelist"],
			description: "The command displays the list of your unloved musics",
			usage: "{{prefix}}topdislike",
			cooldown: 0
		});
	}
	...
	run(client, message, _args) {
		...
		if (data.dislike.length === 0) {
			message.channel.send("There's no music you don't like!");
		} else {
			if(data.dislike.length > 10) {
				let pages = Math.round(data.dislike.length / 10 + 0.49);
				let page = 1;
				let p0 = 0;
				let p1 = 10;
				...
				message.channel.send({
					embed: {
						...
						description: data.dislike.map((i) => `- **${i}**`).slice(0, 10).join("\n"),
						thumbnail: {
